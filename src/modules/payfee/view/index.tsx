...
export default function PayFeesView() {
  ...
  const [expandedMethod, setExpandedMethod] = useState<"card" | "transfer" | null>("card");
  ...
  const countdownState = useCountdown(dvaDetails?.expires_at || "", () => clearDVA());
  ...
  const [makePaymentWithRedirect, { isLoading: isPayingWithRedirect }] = useMakePaymentWithRedirectMutation();

  const students = data?.data?.students ?? [];
  const isPayable = students.flatMap(student => student.items ?? []).length > 0;

  useEffect(() => {
    if (students.length > 0 && !isInitialized) {
      const allIds = students.flatMap(s => s.items.map(i => `${s.id}_${i.paymentItemId}`));
  ...
  const totalPayableAmount = students.reduce((acc, student) => {
    const sum = student.items.reduce((itemSum, item) => 
      selectedItemIds.includes(`${student.id}_${item.paymentItemId}`) ? itemSum + item.amount : itemSum, 0
    );
    return acc + sum;
  }, 0);
  ...
  const handlePayment = async () => {
    ...
    try {
      const studentIdsToPay = Array.from(new Set(selectedItemIds.map(id => id.split("_")[0])));
      const itemIdsToPay = Array.from(new Set(selectedItemIds.map(id => id.split("_")[1])));
  ...
  const transferContent = dvaDetails ? (
    <div className="border-t border-gray-100 bg-gray-50/50 p-6 animate-in fade-in">
      <DVAPaymentContent dvaDetails={dvaDetails} isExpired={countdownState.isExpired} isModal={false} />
    </div>
  ) : (
    <div className="border-t border-gray-100 bg-gray-50/50 p-6 animate-in fade-in flex flex-col items-center justify-center">
      ...
      <p className="text-gray-500 text-center mb-5 max-w-sm">
        Securely generate a designated Virtual Account to proceed with your transfer payment.
      </p>
      <button
  ...
  if (!isPayable) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center space-y-4">
        ...
        <p className="text-gray-500 max-w-md">You do not have any outstanding fee payments at this time. Your wards are fully cleared.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-8 max-w-7xl mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Outstanding Fees</h1>
        <p className="text-gray-500 mt-2">Select the individual fee items you wish to pay for right now.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 xl:col-span-8 flex flex-col gap-6">
          {students.map((student) => {
            ...
            const studentItemIds = student.items.map(i => `${student.id}_${i.paymentItemId}`);
            const allSelected = studentItemIds.every(id => selectedItemIds.includes(id));
            const someSelected = studentItemIds.some(id => selectedItemIds.includes(id));
            
            return (
              <div key={student.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm transition-shadow hover:shadow-md">
                {/* Student Header */}
                <div className="p-5 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {student.items.length} pending {student.items.length === 1 ? 'item' : 'items'}
                      </p>
                    </div>
                  </div>
                  <button 
                  ...
                  >
                    {allSelected ? <CheckCircle2 className="w-4 h-4" /> : <Circle className="w-4 h-4" />}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <button className={`shrink-0 transition-colors ${isSelected ? "text-purple-600" : "text-gray-300"}`}>
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500 mt-0.5">Fee Reference ID: {item.paymentItemId.slice(-6).toUpperCase()}</p>
                          </div>
                        </div>
                        <p className={`font-bold tracking-tight ${isSelected ? "text-purple-700" : "text-gray-900"}`}>
        ...
        <div className="lg:col-span-5 xl:col-span-4 sticky top-6">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/40 overflow-hidden">
            <div className="p-6 border-b border-gray-100 bg-gray-900 text-white">
              ...
              <p className="text-gray-400 text-sm">Review your selection and checkout</p>
            </div>
            
            <div className="p-6 bg-gray-50/30">
              <div className="flex justify-between items-end mb-6">
                <span className="text-gray-500 font-medium">Selected Total</span>
              ...
              <div className="space-y-3">
                ...
                >
                  <div className={`flex items-center justify-center w-5 h-5 rounded-full border ${selectedMethod === 'card' ? 'border-purple-600' : 'border-gray-300'}`}>
                    {selectedMethod === 'card' && <div className="w-2.5 h-2.5 bg-purple-600 rounded-full" />}
                  </div>
                  <CardIcon className="w-6 h-6 text-gray-700" />
                  <span className="font-medium text-gray-900 flex-1 text-left">Pay With Card</span>
                </button>
                ...
                <button
                  onClick={() => {
                    ...
                    setExpandedMethod(expandedMethod === "transfer" ? null : "transfer");
                  }}
                  className={`w-full p-4 flex items-center space-x-4 border rounded-xl transition-all ${
                ...
                >
                  <div className={`flex items-center justify-center w-5 h-5 rounded-full border ${selectedMethod === 'transfer' ? 'border-purple-600' : 'border-gray-300'}`}>
                    {selectedMethod === 'transfer' && <div className="w-2.5 h-2.5 bg-purple-600 rounded-full" />}
                  </div>
                  <BankIcon className="w-6 h-6 text-gray-700" />
                  <span className="font-medium text-gray-900 flex-1 text-left">Bank Transfer</span>
                </button>
              </div>
            </div>
